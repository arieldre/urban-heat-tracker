...
export default function UHFBUploadTab() {
    ...
    for (const { dim, video } of toCreate) {
      try {
        const r = await fetch('/api/fb-control', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'create-ad', game: 'uh', videoId: video.id, videoTitle: video.title, adsetId, message }),
        });
        const data = await r.json();
        if (!r.ok || data.error) throw new Error(data.error || `HTTP ${r.status}`);
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-[300px] shrink-0 border-r border-border flex flex-col">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          ...
          <button
            ...
            disabled={refreshing}
            className="font-mono text-[9px] text-accent border border-[rgba(var(--color-accent-rgb),0.25)] hover:border-accent rounded px-2 py-0.5 cursor-pointer disabled:opacity-40 transition-colors"
          >
            {refreshing ? '…' : '↻'}
          </button>
        </div>
      ...
      {/* Right — active ads (read-only) + upload */}
        ...
        <div className="p-5">
          ...
          {results ? (
            <div className="space-y-3 max-w-[480px]">
              ...
              <button onClick={resetForm} className="font-mono text-[11px] text-accent underline cursor-pointer">
                Upload another
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 max-w-[480px]">
                ...
                >
                  {adsets.map(a => (
                    <option key={a.id} value={a.id}>
                      [{a.platform?.toUpperCase()}] {a.name.replace(/^UH_FB_/i, '').slice(0, 36)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-surface2 border border-border text-text font-mono text-[10px] rounded px-3 py-2 placeholder:text-muted resize-none"
                />
              </div>
